...
void LcdPrintString(const char*, const char*);
...
const char initial_message[LINE][COLUMN] = {"    CENG 336    ", "   TAMAGOTCHI   "};
...
TASK(LCD)
{
    ...
    InitLCD();
    // Greeting message
    LcdPrintString(initial_message[0], initial_message[1]);
...
/**********************************************************************
 * Copy two strings of 16 characters into the LCDchar buffer.
 *
 * @param  first_line  IN  First line of LCD
 * @param  second_line IN  Second line of LCD
 * @return void 
 **********************************************************************/
void LcdPrintString(const char *first_line, const char *second_line)
{
  int i;
  for(i = 0; i < COLUMN; ++i){
    LCDchar[0][i] = *first_line++;
    LCDchar[1][i] = *second_line++;
  }
