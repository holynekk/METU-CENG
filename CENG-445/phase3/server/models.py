...
class Cell:
    ...
    def get_state(self):
        data = dict()
        data["type"] = self.type
        data["players_on"] = self.players_on
        return data
...
class Player:
    ...
    def move(self, cell_count=1, is_teleported=False):
        ...
        """
        self.board_position.players_on.remove(self.user.username)
        ...
        self.board_position.players_on.append(self.user.username)
    ...
    def get_user_state(self):
        ...
        data["picked_cell"] = self.picked_cell
        return data
...
class Board:
    ...
    def detach(self, player):
        ...
        if player in self.player_list:
            if (
                self.turn_ == self.player_list.index(player)
                and self.turn_ == len(self.player_list) - 1
            ):
                self.turn_ = 0
            ...
            if player.user.username in player.board_position.players_on:
                player.board_position.players_on.remove(player.user.username)
    ...
    def ready(self, player):
        ...
        self.initial_cell.players_on.append(player.user.username)
        ...
        if self.player_list.index(player) != self.turn_:
            ...
            if command == COMMAND_CHOICES[COMMAND_PASS][1]:
                ...
                if cell.type == CELL_TYPE_CHOICES[CELL_TYPE_PROPERTY][1]:
                    if type(cell.owner) == int:
                        return f"This propert belongs to another player. You cannot but it!"
    ...
    def getboardstate(self):
        ...
        for _ in range(len(self.cells)):
            cells.append(dummy_cell.get_state())
            dummy_cell = dummy_cell.next_cell

        data = dict()
        data["message"] = (
            f"Waiting for {4 - len(list(filter(lambda player: player.status == USER_STATUS_READY, self.player_list)))} more players to be ready!"
            if self.is_game_over
            else f"It's {self.player_list[self.turn_].user.username}'s turn to play!"
        )
        data["board"] = {
            "id": self.id,
            "name": " ".join(self.name),
            "num_players": len(self.player_list),
            "is_game_started": not self.is_game_over,
        }
    ...
    def getboardinfo(self):
        data = dict()
        data["id"] = self.id
        data["name"] = " ".join(self.name)
        data["num_players"] = len(self.player_list)
        data["is_game_over"] = self.is_game_over
        data["ever_started"] = self.ever_started
        return data
