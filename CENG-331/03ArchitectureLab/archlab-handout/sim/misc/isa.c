...
instr_t instruction_set[] = 
{
    ...
    {"icmpq",   HPACK(I_ICMPQ, F_NONE), 10, I_ARG, 2, 8, R_ARG, 1, 0},
...
stat_t step_state(state_ptr s, FILE *error_file)
{
	...
	(hi0 == I_RRMOVQ || hi0 == I_ALU || hi0 == I_PUSHQ ||
	 ...
	 hi0 == I_MRMOVQ || hi0 == I_LEAQ || hi0 == I_ICMPQ);
	...
	(hi0 == I_IRMOVQ || hi0 == I_RMMOVQ || hi0 == I_MRMOVQ ||
	 hi0 == I_JMP || hi0 == I_CALL || hi0 == I_LEAQ || hi0 == I_ICMPQ);
	...
	s->pc = ftpc;
	break;
    /* Let's try to simulate leaq here! Sure lots of copy-paste... (DS) */
    case I_LEAQ:
	if (!ok1) {
	    if (error_file)
		fprintf(error_file,
