...
# Number: 2381093
#
# Firstly, we need to add ILEAQ into instr_valid array in order to
# set it as another valid instruction. Then, I added ILEAQ into 
# need_regids and need_valC, since leaq instruction takes two 
# registers D_rA and D_rB with one constant value which is called D_ValC.
#
# To decode those registers, I added ILEAQ into relevant places at
# srcA and srcB arrays. Afterward, to set destionation of calculation
# D_ValC + D_rB, I put ILEAQ in a line (icode in { ILEAQ } : D_rA;) which
# is also added by me in the destE array. This way we made sure our 
# D_destE is D_rA.
#
# In execute stage, I passed E_ValC constant into the input A of ALU 
# and E_ValB as the input B of ALU to operate addition on these two
# variables by simply adding ILEAQ into proper places. Since 
# condition code will not change, we don't need to put it in set_cc.
#
# In memory and program counter update stages, we don't need to make
# any changes, since program counter is handled automatically and 
...
#               valB <- R[rB]
#
# Execute       E_ValE <- E_valC OP E_valB
#  
# Memory
# 
# Write Back    R[D_rA] <- valE
# 
# Pc Update     PC <- valA 
####################################################################
####################################################################
